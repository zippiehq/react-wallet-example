...
const WalletComponent = ({ masterseed, userDetails }: { masterseed: string; userDetails: any }) => {
  ...
  useEffect(() => {
    if (!walletLoaded) {
      ;(async () => {
        ...
        await Wallet.ERC20.init(options)
  ...
  const getAccountAddress = async (): Promise<string> => {
    try {
      const options = {}
      const accountAddress = await Wallet.ERC20.getAccountAddress(options)
  ...
  const addAccount = async () => {
    try {
      const options = {
        tokenAddress: '0x89c7a7c02520ade056608484f254b5df1a305a09', // BasicERC20Mock local hardhat network
      }
      const account = await Wallet.ERC20.addAccount(options)
  ...
  const getAccounts = async () => {
    try {
      accounts = await Wallet.ERC20.getAccounts()
  ...
  const getAccountBalance = async () => {
    try {
      ...
      balance = await Wallet.ERC20.getAccountBalance(options)
  ...
  const getAccountTransactions = async () => {
    try {
      ...
      transactions = await Wallet.ERC20.getAccountTransactions(options)
  ...
  const createPayment = async () => {
    try {
      ...
      paymentReceipt = await Wallet.ERC20.createPayment(options)
  ...
  const fetchPayment = async () => {
    try {
      ...
      paymentData = await Wallet.ERC20.fetchPayment(options)
  ...
  const redeemPayment = async () => {
    try {
      ...
      const response = await Wallet.ERC20.redeemPayment(options)
